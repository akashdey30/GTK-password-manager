================================================================================
  ...
  Complete Directory Structure + All File Contents (debugged)
================================================================================
...
│
├── data/
│   ├── vault.dat                # Encrypted credential entries
│   └── master.dat               # Encrypted master password record
│
├── include/
│   ├── credentials.h            # Credential struct definition
│   ├── app_state.h              # AppState struct + MAX_LEN
│   ├── dialogs.h                # GTK dialog function declarations
│   ├── encryption.h             # XOR_KEY constant + xor_buffer() prototype
│   ├── master_auth.h            # Master auth function declarations
│   ├── service_manager.h        # Credential CRUD declarations
│   ├── ui.h                     # UI construction declarations
│   └── vault_storage.h          # Vault file I/O declarations + file path defines
│
└── src/
    ├── encryption.c             # xor_buffer() implementation
    ├── vault_storage.c          # Encrypted vault and master file I/O
    ├── master_auth.c            # Master password logic
    ├── service_manager.c        # Credential add / delete / search
    ├── dialogs.c                # All GTK dialog construction
    ├── ui.c                     # GTK main window + service refresh
    └── main.c                   # Entry point
...
FILE: data/vault.dat
--------------------------------------------------------------------------------
(empty file — 0 bytes, created at runtime when first credential is saved)

--------------------------------------------------------------------------------
FILE: data/master.dat
--------------------------------------------------------------------------------
(empty file — 0 bytes, created at runtime when master password is first set)
...
// Returns true if user entered and confirmed a non-empty password
bool dl_prompt_new_master(GtkWindow *parent, char out_pass[]);

// Prompt user to enter existing master password for verification
// Returns true if user entered a non-empty password
bool dl_prompt_verify_master(GtkWindow *parent, char out_pass[]);

// Add Credential dialog (GTK button callback)
void dl_add_credential_dialog(GtkButton *button, gpointer data);

// Search Credential dialog (GTK button callback)
void dl_search_credential_dialog(GtkButton *button, gpointer data);

// Delete Credential dialog (GTK button callback)
void dl_delete_credential_dialog(GtkButton *button, gpointer data);

// Show all credentials for a service (triggered by service button click)
...
// Handles both first-run setup and subsequent login verification
bool ma_prompt_master_password(GtkWindow *parent, char session_master[]) {
    if (!ma_master_exists()) {
...
================================================================================
  ...
  gcc -Iinclude src/*.c -o password_vault $(pkg-config --cflags --libs gtk+-3.0)
